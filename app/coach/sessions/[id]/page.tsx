...
export default function SessionDetail({ params }: { params: { id: string } }) {
  ...
  .from('ra_sessions')
  .select('*, athlete:ra_athletes(profile:ra_profiles!ra_athletes_profile_id_fkey(full_name, email))')
  .eq('id', params.id)
  .single()
...
console.log('Session error:', JSON.stringify(sessionError))
  ...
  if (loading) return <div style={{ padding: '60px', textAlign: 'center', color: '#4A6880', background: '#0D1B2A', minHeight: '100vh' }}>Loading session...</div>
  if (!session) return <div style={{ padding: '60px', textAlign: 'center', color: '#EF4444', background: '#0D1B2A', minHeight: '100vh' }}>Session not found</div>

  const inputStyle = { background: '#0D1B2A', border: '1px solid #1E3A5F', borderRadius: '6px', color: '#F0F4F8', fontSize: '13px', padding: '6px 10px', outline: 'none' } as React.CSSProperties

  return (
    <div style={{ minHeight: '100vh', background: '#0D1B2A', color: '#F0F4F8' }}>
      {/* Nav */}
      <nav style={{ padding: '0 32px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #1E3A5F', background: '#0A1F4E' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <a href="/coach/programming" style={{ fontSize: '13px', color: '#8BA3BF', textDecoration: 'none' }}>← Calendar</a>
          <span style={{ color: '#1E3A5F' }}>|</span>
          <span style={{ fontSize: '14px', fontWeight: '600' }}>{session.session_name || 'Session'}</span>
        </div>
        <span style={{ fontSize: '13px', color: '#8BA3BF' }}>
      ...
      <div style={{ padding: '28px 32px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        {(session.target_rpe_low || session.coach_notes) && (
          <div style={{ background: '#142236', border: '1px solid #1E3A5F', borderRadius: '10px', padding: '16px 20px', marginBottom: '24px', display: 'flex', gap: '24px' }}>
            {(session.target_rpe_low || session.target_rpe_high) && (
              <div>
                <div style={{ fontSize: '11px', color: '#8BA3BF', textTransform: 'uppercase' as const, letterSpacing: '0.5px', marginBottom: '4px' }}>Target RPE</div>
                <div style={{ fontSize: '18px', fontWeight: '800', color: '#C19B30' }}>{session.target_rpe_low}–{session.target_rpe_high}</div>
              </div>
            )}
            {session.coach_notes && (
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: '11px', color: '#8BA3BF', textTransform: 'uppercase' as const, letterSpacing: '0.5px', marginBottom: '4px' }}>Coach Notes</div>
                <div style={{ fontSize: '13px', color: '#F0F4F8' }}>{session.coach_notes}</div>
              </div>
            )}
          </div>
        )}
        ...
        {blocks.length === 0 ? (
          <div style={{ padding: '60px', textAlign: 'center', color: '#4A6880', border: '1px dashed #1E3A5F', borderRadius: '12px' }}>
            No exercises yet. Click <strong style={{ color: '#C19B30' }}>+ Add Exercise</strong> to build this session.
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {blocks.map((block, idx) => {
              ...
              return (
                <div key={block.id} style={{ background: '#142236', border: '1px solid #1E3A5F', borderRadius: '10px', padding: '16px 20px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                    {/* Index */}
                    <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: '#0A1F4E', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', color: '#8BA3BF', flexShrink: 0, marginTop: '2px' }}>
                      {idx + 1}
                    </div>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                        ...
                        {orm && (
                          <span style={{ fontSize: '11px', color: '#8BA3BF' }}>1RM: {orm}kg</span>
                        )}
                      </div>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '12px', alignItems: 'center' }}>
                        ...
                        {calcWeight && block.load_type === 'percentage' && (
                          <div style={{ marginTop: '16px', padding: '6px 12px', background: 'rgba(193,155,48,0.1)', border: '1px solid rgba(193,155,48,0.2)', borderRadius: '6px' }}>
                            <span style={{ fontSize: '13px', fontWeight: '700', color: '#C19B30' }}>≈ {calcWeight}kg</span>
                          </div>
                        )}
                      </div>
      ...
      {showExercisePicker && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}
          ...
          <div style={{ background: '#142236', border: '1px solid #1E3A5F', borderRadius: '16px', width: '100%', maxWidth: '560px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}
            ...
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #1E3A5F' }}>
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                ...
                style={{ width: '100%', padding: '10px 14px', background: '#0D1B2A', border: '1px solid #1E3A5F', borderRadius: '8px', color: '#F0F4F8', fontSize: '14px', outline: 'none' }} />
            </div>

            {/* Category tabs - only show when not searching */}
            {!search && (
              <div style={{ display: 'flex', gap: '4px', padding: '12px 24px', borderBottom: '1px solid #1E3A5F', overflowX: 'auto', flexShrink: 0 }}>
                {CATEGORIES.map(cat => (
              ...
              {filteredExercises.length === 0 ? (
                ...
                filteredExercises.map(exercise => (
                  <button key={exercise.id} onClick={() => addExercise(exercise)} style={{
                    ...
                    border: 'none', color: '#F0F4F8', cursor: 'pointer', textAlign: 'left' as const,
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexShrink: 0, marginLeft: '12px' }}>
                      {exercise.has_1rm && <span style={{ fontSize: '10px', color: '#C19B30', border: '1px solid rgba(193,155,48,0.3)', padding: '2px 6px', borderRadius: '4px' }}>1RM</span>}
